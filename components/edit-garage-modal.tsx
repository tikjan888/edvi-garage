...
interface EditGarageModalProps {
  isOpen: boolean
  onClose: () => void
  onSubmit: (garageId: string, garageData: Partial<Garage>) => void
  garage: Garage | null
}

export function EditGarageModal({ isOpen, onClose, onSubmit, garage }: EditGarageModalProps) {
  const { t } = useLanguage()
  ...
  // Заполняем форму данными гаража при открытии
  useEffect(() => {
    if (garage) {
      setFormData({
        name: garage.name,
        ...
        partnerName: garage.partnerInfo?.name || "",
        partnerEmail: garage.partnerInfo?.email || "",
        partnerPhone: garage.partnerInfo?.phone || "",
        splitRatio: garage.partnerInfo?.splitRatio || 50,
        partnerNotes: garage.partnerInfo?.notes || "",
      })
    }
  }, [garage])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) newErrors.name = "Название гаража обязательно"
    if (formData.hasPartner && !formData.partnerName.trim()) {
      newErrors.partnerName = "Имя партнера обязательно"
    }
    if (formData.hasPartner && (formData.splitRatio < 0 || formData.splitRatio > 100)) {
      newErrors.splitRatio = "Доля должна быть от 0 до 100"
    }
    ...
    const garageData: Partial<Garage> = {
        ...
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-6 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold">Редактировать гараж</h2>
            </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <Textarea
              ...
              className="mt-1"
              placeholder="Основное место для продажи автомобилей..."
              rows={2}
            />
          </div>
          ...
          <div className="bg-slate-50 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-slate-800">Партнерство</p>
                  <p className="text-xs text-slate-500">
                    {formData.hasPartner ? "Этот гараж имеет партнера" : "Индивидуальное управление"}
                  </p>
                </div>
              </div>
              <Switch
            ...
            {formData.hasPartner && (
              <div className="space-y-3 pt-3 border-t border-slate-200">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Партнер получает {100 - formData.splitRatio}%</p>
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="mt-1"
                    placeholder="Условия партнерства, соглашения..."
                    rows={2}
                  />
                </div>
              </div>
